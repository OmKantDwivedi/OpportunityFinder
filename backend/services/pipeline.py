...
def start_pipeline(name_query: str = "") -> tuple[bool, str | None]:
    """Kick off a run in a background thread. Returns (started, error).

    name_query: if set, the run searches the ENTIRE Levanta catalog for
    products whose title matches (scan_all), instead of using the page-range
    and MAX_PRODUCTS caps.
    """
...
def _run() -> None:
    ...
    name_query = PIPELINE_STATE.get("name_query", "")
    if name_query:
        _set("products", 3,
             f"Step 1/6 - Searching the ENTIRE Levanta catalog for "
             f"\"{name_query}\" products (>= ${min_comm:g}/sale)...")
        products = levanta.fetch_products(
            min_commission=min_comm, name_query=name_query, scan_all=True
        )
    else:
    ...
    _set("products", 10,
         ...
         + (f", title ~ \"{name_query}\"" if name_query else "") + ")")
    if not products:
        raise RuntimeError(
    ...
    with conn:
        # Each run starts from a clean slate - clear previous results so they
        # don't mix with this run's (especially important for name searches).
